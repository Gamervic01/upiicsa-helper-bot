export const administracionIndustrialData = {
  "¿Qué es la Licenciatura en Administración Industrial?": 
    "La Licenciatura en Administración Industrial (LAI) forma profesionales que combinan conocimientos de administración con aspectos técnicos de la industria.\n\n" +
    "🎯 Perfil de Egreso:\n" +
    "- Gestión de procesos industriales\n" +
    "- Administración de recursos\n" +
    "- Planeación estratégica\n" +
    "- Optimización de operaciones\n" +
    "- Desarrollo organizacional\n\n" +
    "📚 Plan de Estudios: <a href='https://www.upiicsa.ipn.mx/oferta-educativa/ver-carrera.html?lg=es&id=58&nombre=Licenciatura-en-Administración-Industrial' target='_blank' rel='noopener noreferrer' class='text-blue-500 hover:text-blue-700 underline'>Ver plan completo</a>",

  "¿Cuál es el plan de estudios de LAI?":
    "El plan de estudios de LAI se compone de 9 semestres:\n\n" +
    "1️⃣ Primer Semestre:\n" +
    "- Álgebra\n" +
    "- Cálculo Diferencial\n" +
    "- Química\n" +
    "- Administración Básica\n" +
    "- Contabilidad Básica\n\n" +
    "2️⃣ Segundo Semestre:\n" +
    "- Cálculo Integral\n" +
    "- Física\n" +
    "- Probabilidad\n" +
    "- Economía\n" +
    "- Derecho Laboral\n\n" +
    "Y así sucesivamente hasta el 9° semestre. Para ver el plan completo, visita: " +
    "<a href='https://www.upiicsa.ipn.mx/oferta-educativa/ver-carrera.html?lg=es&id=58&nombre=Licenciatura-en-Administración-Industrial' target='_blank' rel='noopener noreferrer' class='text-blue-500 hover:text-blue-700 underline'>Plan de Estudios LAI</a>",

  "¿Qué materias tiene LAI?":
    "Las materias de LAI se dividen en áreas:\n\n" +
    "📊 Área Administrativa:\n" +
    "- Administración Básica\n" +
    "- Planeación Estratégica\n" +
    "- Desarrollo Organizacional\n\n" +
    "💰 Área Financiera:\n" +
    "- Contabilidad\n" +
    "- Finanzas\n" +
    "- Costos\n\n" +
    "🏭 Área Industrial:\n" +
    "- Procesos de Manufactura\n" +
    "- Control de Calidad\n" +
    "- Logística\n\n" +
    "🧮 Área Cuantitativa:\n" +
    "- Matemáticas\n" +
    "- Estadística\n" +
    "- Investigación de Operaciones",

  "¿Cuál es el campo laboral de LAI?":
    "Los egresados de LAI pueden trabajar en:\n\n" +
    "🏢 Empresas Privadas:\n" +
    "- Gerencia de producción\n" +
    "- Administración de proyectos\n" +
    "- Gestión de calidad\n" +
    "- Logística y cadena de suministro\n\n" +
    "🏛️ Sector Público:\n" +
    "- Dependencias gubernamentales\n" +
    "- Empresas paraestatales\n" +
    "- Proyectos de desarrollo\n\n" +
    "💼 Consultoría:\n" +
    "- Mejora de procesos\n" +
    "- Desarrollo organizacional\n" +
    "- Gestión del cambio\n\n" +
    "🚀 Emprendimiento:\n" +
    "- Creación de empresas\n" +
    "- Desarrollo de negocios\n" +
    "- Innovación empresarial"
};
